...
/**
 * Problema 11: Maior produto de 4 números próximos em um grid 20x20.
 */
public class Problem11 {
    ...
    public long solve() {
        ...
        String[] rows = gridStr.trim().split("\n");
        ...
        final int ADJACENT = 4;

        // Horizontal
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLUMNS - ADJACENT + 1; j++) {
        ...
        // Vertical
        for (int i = 0; i < ROWS - ADJACENT + 1; i++) {
        ...
        // Diagonal (ascendente)
        for (int i = 0; i < ROWS - ADJACENT + 1; i++) {
            for (int j = ADJACENT - 1; j < COLUMNS; j++) {
                long product = (long)grid[i][j] * grid[i + 1][j - 1] * grid[i + 2][j - 2] * grid[i + 3][j - 3];
                if (product > biggestProduct) {
                    biggestProduct = product;
                }
